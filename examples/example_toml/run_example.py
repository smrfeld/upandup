...
@dataclass
class DataSchemaV1(DataClassTOMLMixin):
    """First version of the data schema.
    """    
    x: int

@dataclass
class DataSchemaV2(DataClassTOMLMixin):
...
@dataclass
class DataSchema(DataClassTOMLMixin):
...
for i,fname in enumerate(["version_DataSchemaV1.toml", "version_DataSchemaV2.toml", "version_DataSchema.toml"]):
